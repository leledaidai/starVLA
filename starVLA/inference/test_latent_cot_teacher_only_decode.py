...
DEFAULT_CHECKPOINT = (
    "results/Checkpoints/"
    "latent_cot_teacher_only_bridge_train_cot_qwen3vl4b_new/"
    "checkpoints/steps_55000_pytorch_model.pt"
)
DEFAULT_SAMPLE_SEED = 42
DEFAULT_SAMPLE_IMAGE_DIR = Path("results/debug/latent_cot_teacher_only_test_samples")
...
def load_teacher_only_debug_checkpoint(
    ...
    *,
    ...
    cfg.cot.eval_mode = "teacher"
    cfg.cot.enable_teacher_cot_loss = True
    cfg.cot.enable_teacher_action_loss = True
    cfg.cot.enable_student_action_loss = False
...
def print_result(
    *,
    ...
    print("\n[generated_teacher_cot]")
    teacher_cot = outputs.get("teacher_cot_texts", [""])[0]
    print(teacher_cot if teacher_cot else "(empty)")
    print(f"\nnormalized_action_shape: {tuple(action.shape)}")
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate teacher-only latent CoT VLA model: extract images, prompts, generate CoT text via VLM autoregression, and predict actions.")
    ...
    parser.add_argument("--max-cot-tokens", type=int, default=512)
...
def main() -> None:
    ...
    model, norm_stats = load_teacher_only_debug_checkpoint(
        checkpoint_path,
        attn_implementation=attn_impl,
