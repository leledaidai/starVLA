...
# 解决 H200 + NCCL 2.21 NVLS 崩溃的核心开关
export NCCL_NVLS_ENABLE=0

# 以下两行建议保留（帮助快速报错，不会影响性能）
...
config_yaml=./starVLA/config/training/starvla_bridge_cot_small_fit_teacher_only.yaml
...
run_id=teacher_only_cot_small_fit_${data_mix}_qwen3vl4b
wandb_project=starVLA_teacher_only_cot_small_fit
...
python starVLA/training/eval_cot_small_fit.py \
  ...
  --output_path ${output_dir}/small_fit_eval_teacher_only.json \
  --summary_jsonl ${output_dir}/small_fit_eval_teacher_only_results.jsonl \
  --run_root_dir ${run_root_dir} \
  --run_id ${run_id} \
  --datasets.vla_data.data_root_dir ${oxe_data_root} \
  --datasets.vla_data.data_mix ${data_mix} \
  --framework.qwenvl.base_vlm ${base_vlm} \
  --small_fit.episode_manifest_path ${manifest_path} \
  --small_fit.predict_mode teacher_debug
